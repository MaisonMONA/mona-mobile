    ...
    boroughBadge() {
      ...
      for (const element of this.userCollection) {
        for (const e of this.badgeTotal) {
          if (
            e.title.fr ===
            Utils.getDiscovery(element.id, element.dType).getBorough()
          ) {
            if (e.id in this.borough) {
              if (this.borough[e.id].count < this.borough[e.id].requireCount)
                this.borough[e.id].count += 1;
              else if (
                this.borough[e.id].count === this.borough[e.id].requireCount
              ) {
        ...
        if (element.dType === "places") countPlaces += 1;
        else if (element.dType === "heritage") countHeritage += 1;
        else if (element.dType === "artwork") {
          countArtwork += 1;
          for (const o in this.owner) {
            if (
              this.owner[o].title ==
              Utils.getDiscovery(element.id, element.dType).getOwner()
            ) {
              console.log("dans if");
