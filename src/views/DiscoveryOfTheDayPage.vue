      ...
      <div class="page-body-background">
        <div class="page-body">
          <div>
            ...
            <p class="subtitle one" v-if="subtitle1.length > 0">
              {{ subtitle1 }}
            </p>
          ...
          <ion-button
            id="infoButton"
            fill="outline"
            :router-link="`/discovery-details/${type}/${discovery.id}`"
...
export default {
  ...
  data() {
    ...
    do {
      ...
      /* if (type === DiscoveryEnum.HERITAGE) */ else
  ...
  methods: {
    myMap() {
      const INITIAL_COORD = [
        this.discovery.location.lng,
        this.discovery.location.lat,
      ];
      useGeographic();
...
#infoButton {
  ...
  width: 42%;
  transition:
    color 0.2s cubic-bezier(0.3, 0.9, 0.9, 0.9),
    --border-color 0.2s cubic-bezier(0.3, 0.9, 0.9, 0.9);
}
