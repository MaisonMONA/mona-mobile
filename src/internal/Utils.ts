...
const downloadImage = async (
  ...
  const blob = await fetch(
    Globals.apiRoutes.getPhotos + `?discovery_id=${id}&type=${type}`,
    {
      headers: {
...
export default {
  ...
  async sendPictureAndDetails(id: number, type: number | string) {
    ...
    const base64Res = await fetch(
      `data:image/${filename.split(".")[1]};base64,${image.data}`,
    );
      ...
      .then((response) => {
        if (response.ok) {
          ...
          response.json().then((json) => {
            console.log(
              `UPLOAD ERR: code ${response.status}, content: ${JSON.stringify(json)}`,
            );
          });
        }
      })
