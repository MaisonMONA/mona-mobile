...
export class UserData {
  ...
  public static async populate() {
    ...
    try {
      ...
      console.log(
        `${this.type} db: error when parsing data from ${this.path} (${err}).`,
      );
  ...
  public static async getFromServer() {
    ...
    await Promise.all([
      Utils.fetchUserDataOnEndpoint(
        Globals.apiRoutes.artworks.upload,
        "artwork",
      ),
      Utils.fetchUserDataOnEndpoint(Globals.apiRoutes.places.upload, "place"),
      Utils.fetchUserDataOnEndpoint(
        Globals.apiRoutes.heritages.upload,
        "heritage",
      ),
    ]);
  ...
  public static sortByDistance() {
    ...
    this.sortedDiscoveriesDistance = this.sortedDiscoveriesDistance.concat(
      PlaceDatabase.getSubset(0, PlaceDatabase.getSize()),
    );
    this.sortedDiscoveriesDistance = this.sortedDiscoveriesDistance.concat(
  ...
  public static getSortedDiscoveriesDistance(sliceA?: number, sliceB?: number) {
    return this.sortedDiscoveriesDistance.slice(
      sliceA || 0,
      sliceB || Infinity,
    );
  }
  private static async buildCache() {
    this.sortedDiscoveries = ArtworkDatabase.getSubset(
      0,
      ArtworkDatabase.getSize(),
    );
    this.sortedDiscoveries = this.sortedDiscoveries.concat(
      PlaceDatabase.getSubset(0, PlaceDatabase.getSize()),
    );
    this.sortedDiscoveries = this.sortedDiscoveries.concat(
      HeritageDatabase.getSubset(0, HeritageDatabase.getSize()),
    ...
    this.sortedDiscoveries.sort((a: Discovery, b: Discovery) => {
      ...
      if (
        (aFirstChar < 48 || aFirstChar > 57) &&
        (aFirstChar < 65 || aFirstChar > 90) &&
        (aFirstChar < 192 || aFirstChar > 383)
      )
      ...
      if (
        (bFirstChar < 48 || bFirstChar > 57) &&
        (bFirstChar < 65 || bFirstChar > 90) &&
        (bFirstChar < 192 || bFirstChar > 383)
      )
        return -1;
  ...
  public static async checkForDBUpdate() {
    const targets = [
      {
        url:
          Globals.apiRoutes.artworks.getUpdate +
          `?date=${this.data.lastServerCheck.artworks}`,
        type: "artworks",
      },
      {
        url:
          Globals.apiRoutes.places.getUpdate +
          `?date=${this.data.lastServerCheck.places}`,
        type: "places",
      },
      {
        url:
          Globals.apiRoutes.heritages.getUpdate +
          `?date=${this.data.lastServerCheck.heritages}`,
        type: "heritages",
      },
    ];
    ...
    for (const target of targets) {
      try {
        ...
        if (response.ok) {
          ...
          /* if (target.type == "heritages") */ else
            HeritageDatabase.insertNewElements(JSON.parse(content).data);

          hasFoundUpdate = true;
        } else {
          console.log(
            `Server error when updating ${target.type} DB: code ${response.status}`,
          );
        }
      } catch (err) {
    ...
    if (hasFoundUpdate) {
      // Rebuild cache when any DB is updated
  ...
  public static tryUploadingPendingDiscoveries() {
    for (const type of Object.keys(this.data.pendingUpload)) {
      ...
      for (const id of this.data.pendingUpload[type]) {
        // For each element of the category's list
  ...
  public static setMapFocus(params: {
    id?: number;
    dType?: string | number;
    active: boolean;
  }) {
  ...
  public static removeTargeted(collectable: Discovery) {
    const type = collectable.dType + "s";
    this.data.targeted[type] = this.data.targeted[type].filter(
      (targetId: number) => targetId != collectable.id,
    );
  ...
  public static isTargeted(id: number, type: number | string): boolean {
    switch (type) {
      ...
      case DiscoveryEnum.ARTWORK: {
        return (
          this.data.targeted.artworks.find((awkId: any) => awkId === id) !==
          undefined
        );
      }
      case "place":
      ...
      case DiscoveryEnum.PLACE: {
        return (
          this.data.targeted.places.find((plId: any) => plId === id) !==
          undefined
        );
      }
      case "heritage":
      ...
      case DiscoveryEnum.HERITAGE: {
        return (
          this.data.targeted.heritages.find((hrtgId: any) => hrtgId === id) !==
          undefined
        );
      }
      case "badge":
  ...
  public static addPendingUpload(id: number, type: number | string) {
    ...
    if (this.isCollected(id, type)) this.data.pendingUpload[key].push(id);
    else
      throw new Error(
        `Cannot add discovery ${type} id=${id}: discovery not collected.`,
      );
  }
  ...
  public static removePendingUpload(id: number, type: number | string) {
    ...
    this.data.pendingUpload[key] = this.data.pendingUpload[key].filter(
      (pendingId: number) => pendingId != id,
    );
    this.updateFile();
  }
}
