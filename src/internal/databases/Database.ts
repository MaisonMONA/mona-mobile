...
import { Artwork, Discovery, Heritage, Place } from "@/internal/Types";
...
export abstract class Database {
  ...
  private static async populateFromServer(): Promise<void> {
    ...
    else if (this.type == "heritages")
      url = Globals.apiRoutes.heritages.download;
    /* (this.type == "badges") */ else url = Globals.apiRoutes.badges.download;
    ...
    for (const element of elements) {
      ...
      if (this.getFromId(newDiscovery.id) != null) {
        ...
        this.data = this.data.filter(
          (elm: Discovery) => elm.id != newDiscovery.id
        );
      }
  ...
  public static getSubset(a?: number, b?: number): Discovery[] {
    if (a == undefined) return this.data;
    else if (b == undefined) return this.data.slice(0, a);
